...
export default function JDChatbotFunctional() {
  ...
  const choose = async (jd: CVFile) => { const requestId = ++jdLoadRequestRef.current; setSelected(jd); setOpen(false); setMessages([hello]); setEditing(false); setExitConfirmOpen(false); setIncludeCompany(false); setDraft(''); setSaved(''); setJDLoadError(null); setJDLoading(true); try { const content = await readRoomFileText(app, jd); if (requestId !== jdLoadRequestRef.current) return; if (!content.trim()) { setJDLoadError('File JD đang trống.'); return; } setDraft(content); setSaved(content); } catch (error) { if (requestId !== jdLoadRequestRef.current) return; setJDLoadError(`Không thể tải nội dung JD: ${describeFeatureError(error, 'không đọc được file JD.')}`); } finally { if (requestId === jdLoadRequestRef.current) setJDLoading(false); } };
